...
/// The `shapes` crate, laid out in its own directory outside the served root and
/// depended on by path: its trait is what the served crate implements.
const SHAPES_MANIFEST: &str =
    "[package]\nname = \"shapes\"\nversion = \"0.1.0\"\nedition = \"2021\"\n";
const SHAPES_SOURCE: &str = "pub trait Shape {\n    fn area(&self) -> u8;\n}\n";
const SHAPES_IMPL: &str = "pub struct Beacon;\n\nimpl shapes::Shape for Beacon {\n    fn area(&self) -> u8 {\n        1\n    }\n}\n";
const SHAPES_LOCK: &str = "\
...
version = \"0.1.0\"
";
const AREA_SYMBOL: &str = "rift://symbol/rust/lib.rs/Beacon::area";
const SHAPES_UNIT: &str = "rift://source/cargo/shapes@0.1.0/src/lib.rs";

/// Renaming an implementation of a dependency's trait method renames the trait's
/// declaration too, and rust-analyzer proposes that edit in the dependency's own
/// file. The server refuses the proposal and names the file by the dependency unit
/// the catalog serves it under, since dependency source is read-only.
#[tokio::test]
async fn rename_into_a_cataloged_dependency_refuses_naming_its_unit() -> TestResult {
    if !engine_live() {
        return Ok(());
    }
    let shapes = tempfile::tempdir()?;
    fs::create_dir_all(shapes.path().join("src"))?;
    fs::write(shapes.path().join("Cargo.toml"), SHAPES_MANIFEST)?;
    fs::write(shapes.path().join("src/lib.rs"), SHAPES_SOURCE)?;
    let manifest = format!(
        "[package]\nname = \"rift_live_fixture\"\nversion = \"0.0.0\"\nedition = \"2021\"\n\
         publish = false\n\n[lib]\npath = \"lib.rs\"\n\n[dependencies]\n\
         shapes = {{ path = '{}' }}\n\n[workspace]\n",
    ...
    let (directory, client, server_task) = served_workspace(
        &[
            ...
            ("Cargo.lock", SHAPES_LOCK),
            ("lib.rs", SHAPES_IMPL),
        ],
        Some(rust_engine_configuration()),
    )
    .await?;
    require_rust_analyzer(directory.path());
    ...
    let request = tool_request(
        "rename_symbol",
        &json!({ "symbol": AREA_SYMBOL, "new_name": "extent" }),
    );
    let structured = call_retrying_acceptance(&client, request).await?;
    assert_eq!(structured["status"], json!("refused"), "{structured:#}");
    assert_eq!(structured["reason"], json!("unsupported"), "{structured:#}");
    let detail = refusal_detail(&structured);
    assert!(
        detail.contains("dependency source") && detail.contains(SHAPES_UNIT),
        "the refusal names the dependency unit the engine's edit would reach: {structured:#}"
    );
    assert_eq!(
        fs::read_to_string(directory.path().join("lib.rs"))?,
        SHAPES_IMPL
    ...
    assert_eq!(
        fs::read_to_string(shapes.path().join("src/lib.rs"))?,
        SHAPES_SOURCE,
        "a refused rename leaves the dependency as it was"
    );
