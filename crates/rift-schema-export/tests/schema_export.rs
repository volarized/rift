...
fn write_request(directory: &tempfile::TempDir) -> TestResult<export::ExportRequest> {
    Ok(export::parse_arguments([
...
fn check_request(directory: &tempfile::TempDir) -> TestResult<export::ExportRequest> {
    Ok(export::parse_arguments([
        "--check".to_owned(),
        directory.path().display().to_string(),
        directory.path().join("plugin").display().to_string(),
...
fn every_served_identity_pattern_spells_its_path_with_the_shared_class() -> TestResult {
    ...
    patterns(
        &serde_json::from_str(&rift_mcp::schema::schema_document())?,
        &mut found,
    );
    patterns(
        &serde_json::from_str(&rift_protocol::schema::configuration_schema_document())?,
        &mut found,
    );
...
fn run_without_arguments_writes_default_output_directory() -> TestResult {
    ...
    assert_eq!(written, rift_mcp::schema::schema_document());
...
fn check_fails_when_configuration_schema_is_stale() -> TestResult {
    ...
    let error =
        export::run(&check_request(&directory)?).expect_err("stale configuration schema must fail");
...
fn configuration_schema_document_is_deterministic() -> TestResult {
    let first = rift_protocol::schema::configuration_schema_document();
    assert_eq!(
        first,
        rift_protocol::schema::configuration_schema_document()
    );
...
fn package_index_schema_document_is_deterministic() -> TestResult {
    let first = rift_protocol::schema::package_index_schema_document();
    assert_eq!(
        first,
...
#[test]
fn global_contract_target_validates_the_published_contract() -> TestResult {
    let request = export::parse_arguments([
        "--global-contract".to_owned(),
...
#[test]
fn global_contract_target_reports_contract_drift() -> TestResult {
    let directory = tempfile::tempdir()?;
...
fn check_fails_when_the_package_index_schema_is_stale() -> TestResult {
    ...
    let error =
        export::run(&check_request(&directory)?).expect_err("a stale package index schema fails");
    let ExportError::CheckMismatch { path } = error else {
        panic!("expected CheckMismatch, got {error:?}");
    };
...
fn the_analyzer_manifest_writes_below_its_root_and_checks_clean() -> TestResult {
    ...
    let written = export::parse_arguments([
    ...
    let checked = export::parse_arguments([
        "--check".to_owned(),
        "--analyzer-manifest".to_owned(),
        root.path().display().to_string(),
    ])?;
    ...
    fs::write(&path, "{}\n")?;
    let error = export::run(&checked).expect_err("a stale manifest fails the check");
...
fn the_analyzer_manifest_names_a_path_it_cannot_write() -> TestResult {
    ...
    let error = export::run(&request).expect_err("a directory takes no document");
...
fn the_analyzer_manifest_over_a_root_without_inputs_names_the_missing_path() -> TestResult {
    ...
    let error = export::run(&request).expect_err("a root holding no inputs renders no manifest");
...
fn plugin_export_writes_the_generated_manifest_and_skill() -> TestResult {
    ...
    assert_eq!(manifest, skill::plugin_manifest());
    let generated = skill::generate(&rift_mcp::schema::tool_listing(), SkillForm::Plugin)?;
...
fn check_fails_when_the_plugin_manifest_is_stale() -> TestResult {
    ...
    let error =
        export::run(&check_request(&directory)?).expect_err("a stale plugin manifest must fail");
    let ExportError::CheckMismatch { path } = error else {
        panic!("expected CheckMismatch, got {error:?}");
    };
...
fn check_succeeds_when_document_matches_served_surface() -> TestResult {
    ...
    export::run(&check_request(&directory)?)?;
    Ok(())
}
...
fn check_error_messages_name_path_and_regenerate_command() -> TestResult {
    ...
    let missing = export::run(&check).expect_err("missing document must fail check");
    assert!(missing.to_string().contains(&document_path));
    assert!(missing.to_string().contains(REGENERATE_COMMAND));

    export::run(&write_request(&directory)?)?;
    fs::write(directory.path().join("public/mcp.json"), "{}")?;
    let mismatch = export::run(&check).expect_err("stale document must fail check");
...
fn write_failed_when_document_path_is_a_directory() -> TestResult {
    ...
    let error = export::run(&request).expect_err("writing over a directory must fail");
    assert!(matches!(error, ExportError::WriteFailed { .. }));
    assert!(std::error::Error::source(&error).is_some());
    Ok(())
}
...
fn schema_document_is_deterministic_and_sorted_by_name() -> TestResult {
    let first = rift_mcp::schema::schema_document();
    let second = rift_mcp::schema::schema_document();
    assert_eq!(
        first, second,
