...
/** `rift:entryPoints`, flattened to the rows the page renders. */
...
export function entryPointRows(seam: EntryPointSeam): EntryPointRow[] {
  const rows: EntryPointRow[] = [];
  const walk = (node: unknown, trail: string[]): void => {
    const target = refName(node as Schema);
    if (target) {
      rows.push({ member: trail[0] ?? "", role: trail.slice(1).join(".") || "—", type: target });
      return;
    }
...
/** Every seam, in file order, tagged with the document that declares it. */
export const entryPointSeams: { file: ProtocolFile; name: string; seam: EntryPointSeam }[] =
  PROTOCOL_FILES.flatMap((file) =>
    Object.entries(documents[file]["rift:entryPoints"] ?? {})
      .filter((entry): entry is [string, EntryPointSeam] => entry[0] !== "description")
      .map(([name, seam]) => ({ file, name, seam })),
  );

// The seam catalogue's own prose. Identical in every document that declares
// entry points, so it is read once, from the one that always has them.
const mcpEntryPoints = documents.mcp["rift:entryPoints"];
export const entryPointsDescription =
  typeof mcpEntryPoints?.description === "string" ? mcpEntryPoints.description : undefined;
export const targetTiers = documents.mcp["rift:targetTiers"];

/**
 * The page's headings, in render order. Mirrors `<ProtocolReference />`
 * exactly — the two are edited together.
 */
export const protocolToc: TableOfContents = [
  { title: "Entry points", url: "#entry-points", depth: 2 },
  ...entryPointSeams.map(({ name }) => ({ title: name, url: `#entry-${name}`, depth: 3 })),
  ...(targetTiers ? [{ title: "Target tiers", url: "#target-tiers", depth: 2 }] : []),
  ...PROTOCOL_FILES.flatMap((file) => [
    { title: `${file}.json`, url: `#${file}-json`, depth: 2 },
    ...defNamesByFile[file].map((name) => ({ title: name, url: `#${name}`, depth: 3 })),
  ]),
  { title: "Index", url: "#index", depth: 2 },
];

/**
 * What search indexes for this page: every heading, plus the prose attached to
 * it. Deliberately excludes property tables, patterns, and raw JSON — they
 * match everything and rank nothing, and they are most of the weight in a
 * pre-rendered Orama index.
 */
export const protocolStructuredData: StructuredData = {
  headings: [
    { id: "entry-points", content: "Entry points" },
    ...entryPointSeams.map(({ name }) => ({ id: `entry-${name}`, content: name })),
    ...(targetTiers ? [{ id: "target-tiers", content: "Target tiers" }] : []),
    ...PROTOCOL_FILES.flatMap((file) => [
      { id: `${file}-json`, content: `${file}.json` },
      ...defNamesByFile[file].map((name) => ({ id: name, content: name })),
    ]),
  ...
  contents: [
    ...PROTOCOL_FILES.map((file) => ({
      heading: `${file}-json`,
      content: documents[file].description,
    })),
    ...defNames.flatMap((name) => {
      const def = defs[name];
      const parts: string[] = [];
      if (def.description) parts.push(def.description);
      for (const [property, value] of props(def)) {
