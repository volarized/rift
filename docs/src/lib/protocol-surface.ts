/**
 * The MCP surface, derived from one version's schemas: what an agent can call,
 ...
 * component bodies — see `pageData` and the merged `allPageData` at the bottom.
 */
...
import { DOC_VERSIONS, type DocVersion } from "@/lib/versions";

const BASE_PATH = process.env.NEXT_PUBLIC_BASE_PATH ?? "";

export function protocolRoot(version: DocVersion): string {
  return `/docs/${version}/protocol`;
}

/** Which page renders a definition's heading. One anchor per definition, per version. */
export function hrefFor(version: DocVersion, name: string): string {
  return `${BASE_PATH}${protocolRoot(version)}/reference/#${name}`;
}

export function pageUrl(version: DocVersion, file: ProtocolFile): string {
  return `${protocolRoot(version)}/${file}`;
}
...
/** One version's derived surface: catalogues, reference grouping, page data. */
export interface Surface {
  mcpTools: ToolEntry[];
  mcpToolGroups: ToolGroupEntry[];
  mcpResources: ResourceEntry[];
  axes: string[];
  axisSummary: Record<string, string>;
  axisOf: Record<string, string>;
  referenceTree: Record<string, ReferenceNode[]>;
  pageData: Record<string, PageData>;
}

function build(version: DocVersion): Surface {
  const protocol: ProtocolData = protocolFor(version);
  const proto: ProtoData = protoFor(version);
  const { documents, defs, defNames, defNamesByFile, axisGroups } = protocol;
  ...
  const mcpTools: ToolEntry[] = seam("mcp", "mcp.tools").map(([name, member]) => {
  ...
  const mcpToolGroups: ToolGroupEntry[] = seam("mcp", "mcp.toolGroups").map(([name, member]) => {
  ...
  const axes: string[] = axisGroups.map((group) => group.name);

  const axisSummary: Record<string, string> = Object.fromEntries(
  ...
  const referenceTree: Record<string, ReferenceNode[]> = (() => {
    ...
    for (const axis of axes) {
      ...
      const buildNode = (name: string, depth: number): ReferenceNode => {
        placed.add(name);
        ...
        if (depth < 3) {
          for (const target of outgoing(name)) {
            if (inbound.has(target) && !placed.has(target))
              children.push(buildNode(target, depth + 1));
          }
        }
      ...
      for (const name of order) {
        if (!placed.has(name)) built.push(buildNode(name, 0));
      }
      trees[axis] = built;
    }
    return trees;
  ...
  // deeper — `buildNode` stops at three levels, so nothing exceeds depth 6.
  ...
  function referencePage(): PageData {
    const outline = axes
      .map((axis) => ({ axis, entries: outlineOf(axis) }))
  ...
  function scipPage(): PageData {
    const declarations = [...proto.scipMessages, ...proto.scipEnums];
    const rpcs = proto.scipServices.flatMap((service) => service.rpcs);
    return {
      toc: [
        ...rpcs.map((rpc) => ({
          title: rpc.name,
...
const surfaces = new Map<DocVersion, Surface>();

/** One version's derived surface, built once per build. */
export function surfaceFor(version: DocVersion): Surface {
  let surface = surfaces.get(version);
  if (!surface) {
    surface = build(version);
    surfaces.set(version, surface);
  }
  return surface;
}

/** Page data across every documented version, keyed by page URL. */
export const allPageData: Record<string, PageData> = Object.assign(
  {},
  ...DOC_VERSIONS.map((version) => surfaceFor(version).pageData),
);
