...
disagree. A read served while the last rebuild's failure stands carries it too, with a `detail`
naming the failure; the next filesystem event retries the rebuild.
