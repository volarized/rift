...
of narrowing which files are searched, and `source` - the excerpt around the hit - is its only
served member.
...
one of those files returns as a `file` hit, tagged `matched_by: ["ranked"]` the same way:
...
A visible file with no extension - `justfile`, `Dockerfile` - joins the same index by default,
since no configured [`[search.text]`](/docs/configuration#search) extension could ever match it:

```json
{ "query": "cargo fmt --all --check" }
```

returns `justfile` the same way: a `file` hit carrying `semantic: false`.
...
`rift://logs` reads the server's own diagnostics back, newest first. Every event the process
emits under the `[logs] capture` filter lands in the workspace database, so an agent that met a
refusal can ask what the server was doing without an operator reading its terminal:
