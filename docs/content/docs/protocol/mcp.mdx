...
| [`search`](#tool-search) | ranked files and symbols across a selected source scope |
...
Every read carries an optional `projection` and an optional `rev`, and reads the workspace tree
when both are absent. The server refuses a read naming a `projection` with
...
`rev` and `projection` never combine: a request naming both refuses as `invalid_request`. A
revision search refuses `force_include`: reaching index-excluded files is a walk of the working
tree, which a revision does not have.
...
`include_history` applies to project symbols and attaches their version-control timeline as a
...
`traversal`. A request naming either refuses with `capability_unavailable`, as does
`target: "node"` and `scope: "dependencies"`.

Search defaults to `scope: "project"`; `scope: "all"` also includes dependency,
standard-library, and external declarations.
...
```json
{
  "hit": {
    ...
    "file": {
      ...
      "content": { "kind": "regular", "size": 3546, "executable": false },
      "languages": [],
      "regions": [],
      "semantic": false
    }
...
Neither hit above carries `source`: a request pays that lookup only when `include` names it.
`include: ["source"]` attaches the excerpt as `source` on every hit; an omitted `include` returns
the hit's symbol or file, `path`, `span`, and `line` alone, as both hits above do.
...
| Resource | Content |
| --- | --- |
| [`rift://workspace{?page_index}`](#resource-workspace) | source catalog, per-language providers, limits, and hooks |
...
The workspace resource pages through the captured source catalog. Each unit identifies its
owner, package, source kind, language, and content digest; resolver summaries explain which
discovery revisions produced that catalog.
...
refuses `unsupported`, naming the policy.
...
change tool produces. A language no [`[engines]`](/docs/configuration#engines) table claims
refuses as `unsupported`, and an engine that declines the new name refuses as
`unmet_precondition` carrying the engine's own words. After the change applies, the server sweeps
the tree for surviving occurrences of the old name and reports each one as a warning.

<McpTool name="move_file" />

An engine advertising `workspace/willRenameFiles` proposes the reference updates, and they land
with the move as one change. Without such an engine the file still moves, and the result carries
a warning saying references were not updated.
...
**Applied**, when it produced edits. The change lands in the targeted tree, and the result's
`summary` carries the change `id`, the changed `paths`, the concrete `edits`, resolution
`diagnostics`, and the `guarantees` the workspace's passing hooks established.

Each entry in `edits` names the bytes it replaced: one entry per hunk for `patch`, one per
engine-proposed edit for `rename_symbol` and `move_file`, and one spanning the whole file for a
file the change created or removed.
