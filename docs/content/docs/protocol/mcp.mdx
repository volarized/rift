...
Every visible bounded UTF-8 file without a NUL byte joins the same index, independent of language
or extension. A hit on one of those files returns as a `file` hit, tagged
...
A visible file with no extension - `justfile`, `Dockerfile` - follows the same rule:
...
changed. The result's `summary` carries change `id`, final changed `paths`, concrete `edits`,
diagnostics, and guarantees passing validation hooks established. Paths and edits are sorted by
project path; transform writes are part of same summary.
...
bytes. Response is `{ "status": "unchanged" }`: no change id, path, edit, index rebuild, or engine
