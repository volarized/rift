...
<VersionBadge version="v0.0.32" languages={["Rust"]} />

`packages` lists what the [dependency resolvers](/docs/architecture#source-discovery) cataloged:
the Cargo resolver reads the resolved graph from `cargo metadata`, and falls back to the
workspace's `Cargo.lock` when `cargo` is not on the server's `PATH`. Each entry is a
[`PackageIdentity`](/docs/protocol/reference#PackageIdentity). The list follows the lockfile:
a `Cargo.lock` rewritten while the server runs is re-cataloged on the next publication.
