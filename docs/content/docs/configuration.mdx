...
A workspace past it refuses its rebuild: a read then answers `temporarily_unavailable` naming
`units_max`, the observed count, and the maximum. Raise the key, or narrow
