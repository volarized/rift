...
`dependencies` shapes the [dependency index](/docs/architecture#dependency-index): whether it
...
- `enabled = false` turns the index off: [`get_symbol`](/docs/protocol/mcp#tool-get_symbol) and
  [`search`](/docs/protocol/mcp#tool-search) refuse a `scope` beyond `project` with
...
[`[source]`](/docs/configuration#source) so fewer files reach the index.
...
`cargo nextest run` compiles the very change it's checking.
